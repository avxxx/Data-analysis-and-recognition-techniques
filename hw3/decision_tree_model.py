...
class DecisionTreeModel:
    ...
    def __init__(self, max_depth: int, min_node_size: int):
        ...
        self.root: DecisionTreeModel.SingleBinaryNode = DecisionTreeModel.SingleBinaryNode()

    class SingleBinaryNode:

        def __init__(self, left_node=None, right_node=None, data_dict=None, list_of_sorted_cat_features=None):
            """
            ...
            self.left: Union[DecisionTreeModel.SingleBinaryNode, int] = left_node
            self.right: Union[DecisionTreeModel.SingleBinaryNode, int] = right_node
        ...
        def set_left_as_node(self, left_node):
            self.left: DecisionTreeModel.SingleBinaryNode = left_node

        def set_left_as_num(self, left_num: int):
            self.left: int = left_num

        def set_right_as_node(self, right_node):
            self.right: DecisionTreeModel.SingleBinaryNode = right_node

        def set_right_as_num(self, right_num: int):
            self.right: int = right_num

    @staticmethod
    def calculate_gini_score(groups: List[List], classes: set) -> float:
        """
        ...
        num_elements: int = sum([len(group) for group in groups])
        ...
        for group in groups:
            ...
            for class_i in classes:
                ...
                p = p / np.double(size)
                score += p * (1 - p)
            gini_score += score * size / float(num_elements)

        return gini_score

    @staticmethod
    def single_split(index: int, threshold: float, data: List):
        """
    ...
    def full_node_split(self, data: List) -> Tuple[Dict[str, Any], List[List[Dict]]]:
        """
        ...
        for row in data:
            for index in range(len(data[0]) - 1):
                if index in self.categorical_feature_index_list:
                    ...
                    groups = self.single_split(index, c, new_data)

                    # Вычисляем для него Джини
                    gini = self.calculate_gini_score(groups, classes)
                else:
                    # Строим конкретное разбиение для текущего значения index и data
                    groups = self.single_split(index, row[index], data)
    ...
    @staticmethod
    def calculate_node_value(group: List):
        """
    ...
    def do_split(self, node: SingleBinaryNode, current_depth: int) -> None:
        """
        ...
        if not left_list or not right_list:
            node.left = node.right = self.calculate_node_value(left_list + right_list)
            return

        # Остановка построения дерева, если глубина достаточно большая
        if current_depth >= self.max_depth:
            node.set_left_as_num(self.calculate_node_value(left_list))
            node.set_right_as_num(self.calculate_node_value(right_list))
            return

        # Вызов рекурсивной части, если не было выполнено одно из условий остановки
        node.left = self.recursive_split(data_list=left_list, depth=current_depth)
        node.right = self.recursive_split(data_list=right_list, depth=current_depth)

    def recursive_split(self, data_list: List, depth: int):
        """
        ...
        # Инициализация узла
        node: Union[DecisionTreeModel.SingleBinaryNode, int]
        ...
        if len(data_list) <= self.min_node_size:
            node: int = self.calculate_node_value(data_list)
        else:
            data_dict, list_of_dicts_to_convert = self.full_node_split(data_list)
            node: DecisionTreeModel.SingleBinaryNode = self.SingleBinaryNode(left_node=None, right_node=None,
                                                                             data_dict=data_dict,
    ...
    def build_tree(self, data: List):
        ...
        """
        root = self.full_node_split(data)
        root_node: DecisionTreeModel.SingleBinaryNode = self.SingleBinaryNode(left_node=None, right_node=None,
                                                                              data_dict=root)
    ...
    def fit(self, x: pd.core.frame.DataFrame, y: pd.core.series.Series):
        ...
        root_1, root_2 = self.full_node_split(list(dataset))
        root_node = self.SingleBinaryNode(left_node=None, right_node=None, data_dict=root_1,
                                          list_of_sorted_cat_features=root_2)
        self.do_split(root_node, 1)
        self.root = root_node
        return root_node

    def single_predict(self, node, row: List):
        """
        Метод для предсказания класса некоторого элемента
        :rtype: DecisionTreeModel.SingleBinaryNode
        :param node: узел
        ...
        if current_value < threshold:
            if type(node.left) == DecisionTreeModel.SingleBinaryNode:
                return self.single_predict(node=node.left, row=row)
            else:
                return node.left
        else:
            if type(node.right) == DecisionTreeModel.SingleBinaryNode:
